...
int main() {
	...
	auto RLS = new ParameterEstimation::RecursiveLeastSquare(2,Lambda);
...
a<<-0.0289386, 	-0.369262, 	0.44767, 	-0.0314016, 	0.381151, 	-0.381077, 	0.055673, 	0.692913, 	0.308997, 	-0.184515, 	-0.586492, 	0.020601;
...
// b <<0.164893, 	0.286424, 	-0.270565, 	0.686873, 	-0.102795, 	0.343893, 	0.0232298, 	-0.136117, 	-0.589154, 	0.182915, 	0.389056, 	0.473262;
	...
	while (runloop) {
		...
		robot->updateModel();
		robot->position(pos_kin, link_name, pos_in_link);
		robot->linearVelocity(vel_kin,link_name, pos_in_link);
		...
		pos_local = R_link.transpose()*pos_kin;	
		vel_local = R_link.transpose()*vel_kin;
		}
		else
		{
...
// RLS->addData(force_moment, accel_kin, avel_kin, aaccel_kin, g_local);
		...
		redis_client.setEigenMatrixDerived(POSITION_KIN_KEY, pos_local);
		redis_client.setEigenMatrixDerived(LINEAR_VEL_KIN_KEY, vel_local);
