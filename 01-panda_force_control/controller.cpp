...
#define  GOTO_CONTACT        1
#define  INTO_CONTACT        2
...
int main() {
	...
	while (runloop) {
		...
		else if(state == INTO_CONTACT)
		{
			...
			posori_task->_desired_position = initial_position + circle_radius * Eigen::Vector3d(sin(2*M_PI*circle_freq*timer.elapsedTime()), 1-cos(2*M_PI*circle_freq*timer.elapsedTime()), 0.0);
