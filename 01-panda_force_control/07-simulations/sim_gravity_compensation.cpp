...
const string robot_file = "../../resources/01-panda_force_control/panda_arm_object.urdf";
...
void simulation(Sai2Model::Sai2Model* robot, Simulation::Sai2Simulation* sim)
{
	...
	// create force sensor
	// ForceSensorSim* force_sensor = new ForceSensorSim(robot_name, "link8", Eigen::Affine3d::Identity(), robot);
	...
	double obj_mass = 1.3;
	Eigen::Vector3d obj_com = Eigen::Vector3d(0.0, 0.0, 0.09);
	...
	while (runloop) {
		...
		gravity_torques += Jacobian_obj_com.transpose() * (- obj_mass * Eigen::Vector3d(0.0,0.0, -9.81));

		// std::cout << "gravity torques: " <<  Jacobian_obj_com.transpose() * (- obj_mass * Eigen::Vector3d(0.0,0.0, -9.81)) << std::endl;

		sim->setJointTorques(robot_name, robot_torques + gravity_torques);

		std::cout << "gravity torques orig: " <<  gravity_torques.transpose() << std::endl;
