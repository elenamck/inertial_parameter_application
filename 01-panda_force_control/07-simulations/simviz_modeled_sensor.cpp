...
const string world_file = "../../resources/01-panda_force_control/world_robot_with_sensor.urdf";
const string robot_file = "../../resources/01-panda_force_control/panda_arm_with_modeled_sensor.urdf";
const string robot_name = "FRANKA-PANDA-OPTO";
...
const string link_name = "svh_hand";
...
void simulation(Sai2Model::Sai2Model* robot, Simulation::Sai2Simulation* sim)
{
	...
	double d_f_y = 20;
	double d_f_z = 25;
	// double d_f_x = 2.5*sqrt(k_f_x*0.033333);
	// double d_f_y = 2.5*sqrt(k_f_y*0.033333);
	// double d_f_z = 2.3*sqrt(k_f_z*0.033333);
	...
	// double d_t_x = 2.5*sqrt(k_t_x*0.0000103029);
	// double d_t_y = 2.5*sqrt(k_t_y*0.0000103029);
	// double d_t_z = 2.3*sqrt(k_t_z*0.0000204167);


	// 2*sqrt(k_t*0.0000103029);
	double k_i_x = 0.02;
	double k_i_y = 0.02;
	double k_i_z = 0.06;
	...
	while (runloop) {
		...
		F_x =  - k_f_x * robot->_q(7) - d_f_x * robot->_dq(7) - k_i_x * integrated_pos_error(0);
		F_y =  - k_f_y * robot->_q(8) - d_f_y * robot->_dq(8) - k_i_y * integrated_pos_error(1);
		F_z =  - k_f_z * robot->_q(9) - d_f_z * robot->_dq(9) - k_i_z * integrated_pos_error(2);

		//F_aux = T_1*F_aux;
		Tau_x =  - k_t_x * robot->_q(10) - d_t_x * robot->_dq(10) - k_i_x * integrated_pos_error(3);
		Tau_y =  - k_t_y * robot->_q(11) - d_t_y * robot->_dq(11) - k_i_y * integrated_pos_error(4);
		Tau_z =  - k_t_z * robot->_q(12) - d_t_z * robot->_dq(12) - k_i_z * integrated_pos_error(5);
		force_virtual << F_x, F_y, F_z, Tau_x, Tau_y, Tau_z;
		cout << "current force/torque: " << force_virtual.transpose() << endl;
		cout << "current displacements: " << robot->_q.tail(6).transpose() << endl;
