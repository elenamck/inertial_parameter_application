...
def PlotCompare(counter, time, x_1, x_2, y_1, y_2, z_1, z_2, title_1, title_2, title_3, label_1, label_2,savename):
...
PlotCompare(1, time, pos[:,0], kf_pos[:,0], pos[:,1], kf_pos[:,1],pos[:,2], kf_pos[:,2],"Position in x","Position in y","Position in z", "Kinematics", "Kalman Filter", "position.png")
PlotCompare(2, time, vel[:,0], kf_vel[:,0], vel[:,1], kf_vel[:,1],vel[:,2], kf_vel[:,2],"Velocity in x","Velocity in y","Velocity in z", "Kinematics", "Kalman Filter",  "velocity.png")
PlotCompare(3, time, acc[:,0], kf_acc[:,0], acc[:,1], kf_acc[:,1],acc[:,2], kf_acc[:,2],"Acceleration in x","Acceleration in y","Acceleration in z", "Sensor", "Kalman Filter", "acceleration.png")
PlotNoLimOne(time, acc_raw[:,0],acc_raw[:,1],acc_raw[:,2], "Raw sensor x", "Raw sensor y", "Raw sensor z", "Elapsed time in s", "Acceleration in g" ,"raw.png")
